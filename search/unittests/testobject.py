...
class TestObject(object):
    ...
    def __str__(self):
        ...
        return f"{self.__class__.__name__}({', '.join(values)}) [{hash(self)}]"
    __repr__ = __str__
...
class PropertyTestObject(TestObject):
    ...
    def __init__(self, **kwargs):
        ...
        # for k, v in self.__class__.__dict__.items():
            # if type(v) is property and k not in kwargs:
                # setattr(self, f'_{k}', None)

    def update(self):
        for k, v in self.__class__.__dict__.items():
            if type(v) is property and f'_{k}' not in self.__dict__:
                setattr(self, f'_{k}', None)
    ...
    def __str__(self):
        ...
        for k, v in self.__class__.__dict__.items():
            if type(v) is property:
                values.append(f'{k}={getattr(self, k, None)}')
        return f"{self.__class__.__name__}({', '.join(values)}) [{hash(self)}]"
